  ...
  try {
    ...
    for (const a of data.appearances || []) {
      appearances[a.id] = {
        ...
        matchup: a.match_title || a.title || '',
        gameTime: a.scheduled_at || a.start_time || '',
      }
    }
    ...
    for (const line of data.over_under_lines || []) {
      ...
      if (!opt) continue
      const apId = line.appearance_stat?.appearance_id
      const ap = appearances[apId] || {}
      const pl = players[ap.playerId] || {}
      const lineVal = parseFloat(opt.over_under ?? opt.line ?? 0)
      if (isNaN(lineVal) || lineVal <= 0) continue
      lines.push({
        book: 'Underdog',
        player: pl.name || 'Unknown',
        team: pl.team || '',
        league: ap.league || 'Unknown',
        stat: line.appearance_stat?.display_stat || line.stat_value || '',
        line: lineVal,
        odds: -110,
        matchup: ap.matchup || '',
